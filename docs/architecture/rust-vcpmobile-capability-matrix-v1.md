...
| Conversation truth | Conversation -> MessageNode -> UIMessage -> ordered parts | session metadata only | file-backed topic/history, weaker truth model | upstream request pipeline, not client truth | Rust conversation/node/variant/part already exists | keep current Rust truth, extend toward rib richness |
