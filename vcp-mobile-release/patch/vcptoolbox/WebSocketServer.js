...
const mobileMessageQueue = []; // 移动端离线消息队列（内存，重启清空）
const MOBILE_QUEUE_MAX = 200; // 最多缓存 200 条离线消息
...
function initialize(httpServer, config) {
    ...
    // WebSocket 协议层 ping，防止反向代理空闲超时断开
    setInterval(() => {
        mobileClients.forEach((clientWs, clientId) => {
    ...
    httpServer.on('upgrade', (request, socket, head) => {
        ...
        const mobilePathRegex = /^\/vcp-mobile\/VCP_Key=(.+)$/;
        ...
        const mobileMatch = pathname.match(mobilePathRegex);
        ...
        if (vcpMatch && vcpMatch[1]) {
           ...
           writeLog(`Temporary ChromeControl client attempting to connect.`);
            ...
            connectionKey = mobileMatch[1];
            writeLog(`MobileClient attempting to connect.`);
        } else {
        ...
        if (isAuthenticated) {
            wssInstance.handleUpgrade(request, socket, head, (ws) => {
                ...
                } else if (clientType === 'MobileClient') {
                   mobileClients.set(clientId, ws);
                   writeLog(`MobileClient ${clientId} connected.`);
                   console.log(`[WebSocketServer] 📱 MobileClient ${clientId} connected.`);
                   // 发送离线消息队列
                   if (mobileMessageQueue.length > 0) {
                       const pending = [...mobileMessageQueue];
                       mobileMessageQueue.length = 0;
                       pending.forEach(msg => {
                           if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify(msg));
                       });
                       console.log(`[WebSocketServer] 📱 Flushed ${pending.length} offline messages to MobileClient ${clientId}.`);
                   }
                } else {
    ...
    wssInstance.on('connection', (ws, request) => {
        ...
        } else if (ws.clientType === 'MobileClient') {
            ws.send(JSON.stringify({ type: 'connection_ack', message: 'WebSocket connection successful for MobileClient.' }));
        }
            ...
            try {
                ...
                if (ws.clientType === 'DistributedServer') {
                    ...
                    if (parsedMessage.type === 'heartbeat') {
                        ...
                        writeLog(`MobileClient ${ws.clientId} sent chat message.`);
                    }
                } else {
                    // 未来处理其他客户端类型的消息
                }
        ...
        ws.on('close', () => {
            if (ws.clientType === 'DistributedServer') {
              ...
              console.log(`[WebSocketServer] 📱 MobileClient ${ws.clientId} disconnected.`);
              writeLog(`MobileClient ${ws.clientId} disconnected and removed.`);
           } else {
...
// 广播给移动端客户端（在线直推，离线入队列）
function broadcastToMobile(data) {
    if (!wssInstance) return;
    const messageString = JSON.stringify(data);
    let sent = false;
    
    mobileClients.forEach(clientWs => {
        if (clientWs.readyState === WebSocket.OPEN) {
            clientWs.send(messageString);
            sent = true;
        }
    ...
    // 没有在线的移动端客户端，消息入离线队列
    if (!sent) {
        mobileMessageQueue.push(data);
        if (mobileMessageQueue.length > MOBILE_QUEUE_MAX) {
            mobileMessageQueue.shift(); // 超出上限丢弃最旧的
        }
        console.log(`[WebSocketServer] 📱 No mobile client online, queued message (${mobileMessageQueue.length}/${MOBILE_QUEUE_MAX}).`);
    }
}
...
function getMobileStatus() {
    return {
        online: mobileClients.size > 0,
        clientCount: mobileClients.size,
        queuedMessages: mobileMessageQueue.length
    };
}
