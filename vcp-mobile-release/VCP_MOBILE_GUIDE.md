...
| **AI 按钮交互** | 支持点击 AI 输出的按钮，自动发送对应指令，与 VCPChat 桌面端一致 |
...
### v1.4.1 (2026-02-23)
- **新增**：AI 按钮交互（点击 AI 输出的按钮自动发送指令，兼容 VCPChat 桌面端）
- **新增**：onclick 按钮安全执行（input/alert/confirm/prompt）
- **修复**：按钮无法点击问题（onclick 按钮不再错误触发 iframe 沙箱）
